...
import utils
import pose_analyze as analyze


def run(estimation_model: str, tracker_type: str, classification_model: str,
        label_file: str, camera_id: int, width: int, height: int, action: str, times: int) -> None:
  ...
  Args:
    ...
    classification_model: Name of the TFLite pose classification model.(Optional)
    ...
    action: target action to be detected.
    times: times to repeat the action.
  """
  ...
  # Initialize a deque with a certain length
  list_persons_history = deque(maxlen=1000)
  action_count = 0
  ...
  while cap.isOpened():
    ...
    cv2.putText(image, action_count_text, (left_margin, 4 * row_size), cv2.FONT_HERSHEY_PLAIN,
                font_size, text_color, font_thickness)
    cv2.putText(image, correction_info_text, (left_margin, 5 * row_size), cv2.FONT_HERSHEY_PLAIN,
                font_size, text_color, font_thickness)
    ...
    # Stop the program if the action is completed
    if action_count >= times:
      completed_text = 'Completed!'
      text_location = (left_margin, 6 * row_size)
      cv2.putText(image, completed_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                  font_size, text_color, font_thickness)
      break
...
def main():
  ...
  parser.add_argument(
      '--action', 
      help='target action to be detected.',
      required=False,
      default='')
  parser.add_argument(
      '--times', 
      help='times to repeat the action.',
      required=False,
      type=int,
      default=15)
